...
The release data, result tables, preprocessing pipelines, and final Step7
models are stored with Git Large File Storage (Git LFS).

The large intermediate `.joblib` files produced by Step5 and Step6 are not
bundled in the public release. They remain in the maintainer's local working
copy and are regenerated by the corresponding notebooks.

### Clone with Git LFS

Install Git LFS before cloning, then pull the actual files:
...
If a file still contains pointer text after cloning, run:
...
`Code → Download ZIP` creates a source archive of the selected branch or
commit. Depending on the repository's `Include Git LFS objects in archives`
setting, the archive may contain either the actual bundled LFS files or small
pointer files. Use the Git LFS workflow above when you need the actual data.

### Reproduce the Step5–Step6 analyses

Run the notebooks in numerical order. Step5 generates the intermediate model
artifacts consumed by Step6; Step6 then regenerates its own explainability and
ablation artifacts. The final Step7 models remain bundled because Step8 and
Step9 use them directly.

> **Manifest note:** Some result manifests retain maintainer-side paths as
> provenance. Those paths are not expected to resolve in a fresh clone until
> the corresponding notebook regenerates the local artifacts.
---
